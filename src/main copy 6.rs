...
use core::arch::asm;
use core::ptr;
use cortex_m::peripheral::{SYST, NVIC, DWT};
...
pub unsafe extern "C" fn __cortex_m_rt_TIM2_trampoline() {
   ...
   unsafe{asm!("cpsid i; mov r0,sp; mov r1, lr; bl _timer2;", options(noreturn))};
}
// #[export_name = "TIM2"]
// pub unsafe extern "C" fn __cortex_m_rt_TIM2_trampoline() {
...
pub unsafe extern "C" fn _timer2(){
    ...
    if lr_value == 0xFFFF_FFF9{
        ...
        unsafe{asm!(" pop {{r0-r3, r12, r5, r6, r5}};mov pc, r6 ;nop;", options(noreturn))};
    }
    asm!(
        "MSR msp, R0",  // Move the value from R0 into the PSP (Process Stack Pointer)
        in("r0") r0_value
