...
// Function to enter privileged mode
pub fn enter_privileged_mode() {
    ...
    // Clear the nPRIV bit to enter privileged mode
    let new_control = control & !0x1;

    // Write the updated value back to CONTROL register
    unsafe {
        asm!(
            "MSR CONTROL, {0}",
            in(reg) new_control,
